...
import time
from gotimovement import goti
from laddercheck import ladders
from snakecheck import snakes
from mousepos import * 
from dicemod import dice
from muteunmute import *
...
def credit():
    while True:
        ...
        mouse=pygame.mouse.get_pos()
        click=pygame.mouse.get_pressed()
        if button("Back",mouse[0],mouse[1],w/2-100,700,200,50,b_red,b_red,25,20):
            main()
...
def main():
    ...
    while menu:
        ...
        GD.blit(menubg,(0,0))
        button("Play",mouse[0],mouse[1],(w/2-100),h/2,160,100,grey,black,60,1)

        button("Quit",mouse[0],mouse[1],(w/2-100),(h/2)+160,160,100,grey,black,60,0)

        mouse=pygame.mouse.get_pos()
        if button2("Mute Music",mouse[0],mouse[1],288,180,200,50,grey,black,25):
            pygame.mixer.music.pause()
        if button2("Play Music",mouse[0],mouse[1],588,180,200,50,grey,black,25):
            pygame.mixer.music.unpause()
        if button2("Credits",mouse[0],mouse[1],888,180,200,50,grey,black,25):
            credit()
...
def options():
    ...
    while flag==True:
        ...
        b1=button("Single Player",mouse[0],mouse[1],(w/2-150),250,300,50,grey,black,30,"s")
        #2 player button
        b2=button("2 Players",mouse[0],mouse[1],(w/2)-150,350,300,50,grey,black,30,2)
        #3 player
        b3=button("3 Players",mouse[0],mouse[1],(w/2)-150,450,300,50,grey,black,30,3)
        #4 player
        b4=button("4 Players",mouse[0],mouse[1],(w/2)-150,550,300,50,grey,black,30,4)
        #Back button
        b5=button("Back",mouse[0],mouse[1],30,650,200,50,b_red,b_red,30,5)
        if b5==5:
            main()
        if b1=="s":
            play(21)
